    ...
    <>
      <div className="fixed top-0 left-0 right-0 z-50 flex justify-center pointer-events-none w-full">
        <motion.header
          ...
          className={`mt-1 mx-4 pointer-events-auto transition-all duration-500 ease-in-out flex justify-center relative ${
            isScrolled
              ? "w-[95%] max-w-5xl rounded-full bg-background/40 backdrop-blur-2xl border border-cyan-500/20 shadow-[0_8px_32px_rgba(0,0,0,0.4)] py-1 px-4"
              : "w-full max-w-7xl rounded-2xl bg-white/[0.02] backdrop-blur-md border border-white/5 py-1.5 px-6"
          }`}
        >
          <nav className="flex items-center justify-between gap-4 w-full relative">
            ...
            <div className="flex md:hidden items-center justify-between w-full relative h-8 px-2">
              <div className="z-20">
                <Button
                  size="icon"
                  variant="ghost"
                  className="w-7 h-7 rounded-full pointer-events-auto hover:bg-white/10"
                  onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
                  data-testid="button-mobile-menu"
                >
                  {isMobileMenuOpen ? <X className="w-3 h-3 text-white" /> : <Menu className="w-3 h-3 text-white" />}
                </Button>
              </div>
              ...
              <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-auto z-10">
                ...
                >
                  ...
                  <img 
                    src={logoImg} 
                    alt="MP" 
                    className="relative w-6 h-6 object-cover rounded-full border border-white/30 shadow-2xl" 
                  />
                </motion.div>
              </div>

              <div className="z-20">
                <div className="h-7 w-7 rounded-full bg-gradient-to-br from-slate-400 via-slate-200 to-slate-500 p-[1px] group opacity-100 transition-all duration-300 active:scale-95 shadow-[0_0_15px_rgba(148,163,184,0.3)]">
                  <button
                    onClick={() => {
                      window.dispatchEvent(new CustomEvent('open-ai-chat'));
                    }}
