...
import socksHeroImg from "@assets/generated_images/classic_gentleman_socks_gift_box..png";
...
const mobileStarPositions = [
  { x: 10, y: 20 },    // 0
  { x: 50, y: 75 },    // 1 - фитнес студия
  { x: 35, y: 27 },   // 2 - Интернет магазин косметики
  { x: 50, y: 20 },   // 3 - Streetwear (Интернет-магазин одежды)
  { x: 20, y: 37 },   // 4 - socks (Интернет-магазин носков)
  { x: 60, y: 35 },   // 5 - Туристическое агентство
  { x: 15, y: 86 },   // 6 - Барбершоп
  { x: 15, y: 50 },   // 7 - Стоматологическая клиника
  { x: 65, y: 50 },   // 8 - Ремонт квартир
  { x: 32, y: 58 },   // 9 - Сайт-визитка фотографа
  { x: 66, y: 65 },   // 10 - Автосервис
  { x: 25, y: 96 },   // 11 - Агентство недвижимости
  { x: 15, y: 71 },   // 12 - Салон красоты
  { x: 67, y: 86 },   // 13 - Платформа онлайн-курсов
  { x: 40, y: 28 },   // 14
  { x: 60, y: 45 },   // 15
];
...
function Nebulae() {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none bg-[#0a0a0a]">
      ...
      <div className="absolute inset-0 top-[179px] md:top-[116px]">
        <img
  ...
  return (
    <section id="portfolio" className="relative overflow-hidden bg-[#0a0a0a] pt-4 md:pt-6" style={{ height: isMobile ? "auto" : "100vh", minHeight: isMobile ? "750px" : "800px" }}>
      ...
      <div className="relative z-10 h-full flex flex-col" ref={ref}>
        ...
        <div className="absolute top-0 left-0 right-0 z-30 pt-0 px-6 pointer-events-none">
